 /*config = Npm.require('./config'),
  ...
  _upload = function(response, file) {
  ...
  serveStatic = function(response, pathname) {
  ...
  ifWin = function(response, files) {
    ...
    var mergedFile = __dirname + '\\uploads\\' + files.audio.name.split('.')[0] +
      '-merged.webm';
    ...
    var command = merger + ', ' + audioFile + " " + videoFile + " " +
      mergedFile + '';
  ...
  ifMac = function(response, files) {
    ...
    var command = "ffmpeg -i " + audioFile + " -itsoffset -00:00:01 -i " +
      videoFile + " -map 0:0 -map 1:0 " + mergedFile;
